...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"05.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"06.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"07.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"08.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"09.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"10.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"11.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"12.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"13.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"14.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"15.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"16.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"17.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"18.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"19.tif"))[[2]]
...
for (i in 1:12) {
  ...
  aod <- list()
  aod.month <- stack(paste("Satellite AOD/AOD",i,"20.tif"))[[2]]
...
saveRDS(Pollution, "Data/PollutionAOD55.rds")

setwd("C:/Users/air21/OneDrive - Imperial College London/Documents/PhD/R/Objective 1")


Pollution <- Pollution[,c("Site Name", "time", "NO2", "O3", "PM25", "SO2", "aod.points")]
names(Pollution)[7] <-  "aod.points55"

climate <- readRDS(file = "Data/climatefinal.rds")
climate <- climate[,c("Site Name", "time", "temp", "humid", "precip")]
climate <- unique(climate)
Climate.Pollution <- merge(climate, Pollution, by = c("Site Name", "time"))
...
pops <- raster::extract(pop.r, sites[,c(1,2)])
pop.points <- cbind(pops, sites)
pop.points <- pop.points[,c("pops", "Site Name")]
names(pop.points)[1] <- "population"

Climate.Pollution.Population <- merge(Climate.Pollution, pop.points, by = c("Site Name"), all.x = TRUE)
...
for(i in 1:16){
  PCM.r <- stack("Data/pcm.tif")[[i]]
  PCM.r <- projectRaster(PCM.r, crs = "+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.999601272 +x_0=400000 +y_0=-100000 +ellps=airy +units=m +no_defs")
  
  pcm.sites <- raster::extract(PCM.r, sites[,c(1,2)])
  
  year <- rep(i + 2004, n.sites)
  pcm.points <- cbind(pcm.sites, sites, year)
  pcm[[i]] <- pcm.points[,c("pcm.sites", "Site Name", "year")]
}

pcm <- rbind(pcm[[1]], pcm[[2]], pcm[[3]], pcm[[4]], pcm[[5]], pcm[[6]], pcm[[7]], pcm[[8]], pcm[[9]], pcm[[10]], pcm[[11]], pcm[[12]], pcm[[13]], pcm[[14]], pcm[[15]], pcm[[16]])
names(pcm)[1] <- "pcm"
...
Climate.Pollution.Population$year <- ceiling(Climate.Pollution.Population$time/12) + 2004
  
Climate.Pollution.Population.PCM <-merge(Climate.Pollution.Population, pcm, by = c("Site Name", "year"))

Climate.Pollution.Population.PCM <- merge(Climate.Pollution.Population.PCM, AURNSites, by = "Site Name")

saveRDS(Climate.Pollution.Population.PCM, "Data/All Site Data.rds")


Pollution <- Pollution[,c("Site Name", "time", "aod.points55")]
names(Pollution)[3] <-  "AOD55"
All <- readRDS("Data/All Site Data.rds")
All <- merge(All, Pollution, by = c("Site Name", "time"))
saveRDS(All, "Data/All Site Data.rds")
