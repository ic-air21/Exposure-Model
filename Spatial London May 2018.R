...
london.0518 <- subset(london, year == 2018 & month == 5)
saveRDS(london.0518, "Data/London May 2018.rds")

london.0518 <- readRDS("Data/London May 2018.rds")
xy <- london.0518[,c("Easting", "Northing")]
london.sp <- SpatialPointsDataFrame(coords = xy,  data = london.0518, proj4string = CRS("+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.999601272 +x_0=400000 +y_0=-100000 +ellps=airy +units=m +no_defs"))

london.sp <- subset(london.sp, !is.na(NO2))

bubble(london.sp, "NO2", maxsize = 2)
summary(london.sp$NO2)
hist(london.sp$NO2)
#ggpairs(london.sp[,5:16])
...
bubble(london.sp, zcol="NO2", maxsize=2, sp.layout=list("sp.polygons", London, col="red"))
...
hist(london.sp$NO2)
hist(london.sp$temp)
hist(london.sp$precip)
hist(london.sp$humid)
hist(london.sp$`PM25 PCM`)
hist(london.sp$`NO2 PCM`)
hist(london.sp$AOD47)
hist(london.sp$AOD55)
hist(london.sp$`Satellite NO2`)
hist(london.sp$population)
hist(london.sp$`Altitude (m)`)

coords <- as.matrix(london.sp@coords, nrow = 13)
...
names(london.sp)[c(14,16,44)] <- c("PM25PCM", "SNO2", "NO2PCM")

m1 <- spLM(NO2 ~ temp + precip + humid + NO2PCM + SNO2 + population, data = london.sp, coords=coords, starting=starting,
...
#predictive process
london.sp[c(5:16,28,32:38)]
cov(as.data.frame(london.sp[c(5:16,28,32:38)]))
...
n.samples <- 20000
p <- 7
model2 <- NO2 ~ temp + precip + humid + NO2PCM + SNO2 + population
starting2 <- list("phi" = 3/500, "sigma.sq" = 0.5, "tau.sq" = 0.05)
tuning2 <- list("phi" = 0.1, "sigma.sq" = 0.0001, "tau.sq" = 0.1)
priors2.1 <- list("beta.Norm"=list(rep(0,p), diag(1000,p)),
                  "phi.Unif"=c(3/1000, 3/100), "sigma.sq.IG"=c(2, 1),
                  "tau.sq.IG"=c(2, 0.1))
priors2.2 <- list("beta.Flat", "phi.Unif"=c(3/1, 3/0.1),
                  "sigma.sq.IG"=c(2, 2), "tau.sq.IG"=c(2, 0.1))

London.spLM <- spLM(model2, coords = coords, knots = cd.knots, data = london.sp, 
                starting=starting2,
                tuning=tuning2,
                priors=priors2.1, 
...
thinning <- 100
burn.in <- 0.3*n.samples
m.2 <- spRecover(m2, start=burn.in, thin=thinning, verbose=FALSE) 
theta.samples <- m.2$p.theta.recover.samples

plot(theta.samples[,1:3], auto.layout=TRUE, density=TRUE)
autocorr.plot(theta.samples[,1:3])

beta.samples <- m.2$p.beta.recover.samples
plot(beta.samples[,1:7], auto.layout=TRUE, density=TRUE)
autocorr.plot(beta.samples[,1:7])

round(summary(m.2$p.beta.recover.samples)$quantiles[,c(3,1,5)],3)

w.samples <- m.2$p.w.recover.samples
w.hat.mu <- apply(w.samples,1,mean)
w.hat.sd <- apply(w.samples,1,sd)
surf.mu <- mba.surf(cbind(coords, w.hat.mu), no.X=40, no.Y=40, extend=FALSE)$xyz.est
z.lim <- range(surf.mu[[3]], na.rm=TRUE)
...
London.spLM <- spRecover(London.spLM, start=burn.in)
round(summary(mcmc(cbind(London.spLM$p.beta.recover.samples, London.spLM$p.theta.recover.samples)))$quantiles[,c(1,3,5)],3)

fitted2 <- spPredict(London.spLM, start=burn.in, thin=10, pred.covars=London.spLM$X, pred.coords=London.spLM$coords)
fitted2
...
pred.covars <- London.grids[,c("temp", "precip", "humid", "NO2PCM", "SNO2", "population")]
